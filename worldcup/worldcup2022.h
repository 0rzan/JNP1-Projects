...
public:
	...
	unsigned short roll() {
		short sum = 0;
		for (auto die : dice) sum += die->roll();
		return sum;
	}
			...
			for (std::shared_ptr<Player> player : players) {
				if (!player->isBankrupt()) {
					...
					if (!player->isSuspended()) { // robi ruch
						unsigned short rolled = dice->roll();
						unsigned short position;
						for (unsigned short index = 1; index < rolled; index++) {
                ...
                std::cout<<player->getName()<<" "<<player->getStatus()<<" "<<board->getSquare(player->getPosition())->getName()<<" "<<player->getMoney()<<'\n';
